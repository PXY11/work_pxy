...
class HoldStrategy(PoolSimiulation):
    ...
    def on_time(self, data: dict):
        ...
        ts = data["timestamp"]
        trend = data['trend']
        if trend==True and not self.position_id:
            print('***************************************************【创建池子】***************************************************')
            self.upper_price = price*1.25
            self.lower_price = price*0.8
            self.swap(1, pct=0.5) #一部分usdt换成eth，token0表示eth，token1表示usdt
            #self.swap(0, pct=0.5)
            ...
            if self.price_in_range(price):
                ...
                print('amount_t0:', t0, 'amount_t1:', t1)
